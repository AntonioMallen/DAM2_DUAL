...
public class Paddle : MonoBehaviour 
{
	...
	void Update ()
	{
		if(canMove){															//Is the paddle able to move?
			if(Input.GetKey(KeyCode.UpArrow)){								//Is the left arrow key currently being pressed
				rig.velocity = new Vector2(-1 * speed * Time.deltaTime, 0);		//Set the paddle's rigidbody velocity to move left
			}
			else if(Input.GetKey(KeyCode.DownArrow)){							//Is the right arrow key currently being pressed
