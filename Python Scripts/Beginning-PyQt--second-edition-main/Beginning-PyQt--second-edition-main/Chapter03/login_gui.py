        ...
        try:
            with open(file, 'r') as f:
                for line in f:
                    ...
                    if(len(user_info[1].strip('\n'))>78):
                        str_decoded = cryptocode.decrypt(user_info[1].strip('\n'),"contrasenaSecreta")
                    else:
                        str_decoded=user_info[1].strip('\n')
                    users[username_info] = str_decoded
            ...
            if (username, password) in users.items():
                ...
                QMessageBox.information(self, "Bien hecho!", 
                    "Bien hecho!", QMessageBox.StandardButton.Ok, 
                    QMessageBox.StandardButton.Ok)
